...
import { formatTanggalIndo, formatHpDisplay, getPangkatLengkap, exportExcelSekolah, exportExcelDinas, compareSchoolNames, compareSchoolsByJenjangAndName } from "./utils";
import { PapanInformasiView } from "./components/PapanInformasiView";
import { SchoolIdentityView } from "./components/SchoolIdentityView";
import { SchoolManagerView } from "./components/SchoolManagerView";

// Lucide icon imports
import { 
  Plus, Search, RefreshCw, FileSpreadsheet, Copy, Settings,
  ...
  MessageCircle, ChevronLeft, ChevronRight, Megaphone, Building2
} from "lucide-react";
...
export default function App() {
  ...
  const [activeTab, setActiveTab] = useState<"dashboard" | "data" | "informasi" | "identitas" | "sekolah">("dashboard");
  ...
  const fetchMetadata = async () => {
    try {
      ...
      const rawSchools: School[] = dropData.sekolahs || [];
      rawSchools.sort((a, b) => {
  ...
  const filteredAndSortedGtkList = useMemo(() => {
    ...
    if (searchTerm.trim()) {
      ...
      result = result.filter(item => {
        const nameMatch = item.Nama ? item.Nama.toLowerCase().includes(s) : false;
        const nipMatch = item.NIP ? String(item.NIP).toLowerCase().includes(s) : false;
        const nikMatch = item.NIK ? String(item.NIK).toLowerCase().includes(s) : false;
        return nameMatch || nipMatch || nikMatch;
      });
    }
    ...
    if (isDinas && filterJenjang) {
      result = result.filter(item => {
        ...
        if (filterJenjang === "TK") {
          return /\b(TK|TKN|TKS|PAUD|KB|BA|RA)\b/i.test(sName) || sName.startsWith("TK") || sName.startsWith("TKN") || sName.startsWith("TKS") || sName.startsWith("PAUD");
        }
        if (filterJenjang === "SD") {
          return /\b(SD|SDN|SDS)\b/i.test(sName) || sName.startsWith("SD") || sName.startsWith("SDN") || sName.startsWith("SDS");
        }
        if (filterJenjang === "SMP") {
          return /\b(SMP|SMPN|SMPS)\b/i.test(sName) || sName.startsWith("SMP") || sName.startsWith("SMPN") || sName.startsWith("SMPS");
        }
        return true;
      });
    }
    ...
    if (isDinas && filterBeban) {
      if (filterBeban === "Kepsek_ALL") {
        ...
        result = result.filter(item => item.Beban_Tugas === "Guru Kelas");
      } else if (filterBeban === "Guru Kelas TK") {
        result = result.filter(item => item.Beban_Tugas === "Guru Kelas TK");
      ...
      } else if (filterBeban === "Lainnya") {
        result = result.filter(item => {
          const t = item.Beban_Tugas || "";
          const isGuruKelas = t.startsWith("Guru Kelas");
          const isGuruMapel = t.startsWith("Guru Mapel") || t === "Guru PAI" || t === "Guru PJOK" || t === "Guru Bahasa Inggris";
          const isKepsek = t === "Kepala Sekolah" || t === "PLT. Kepala Sekolah";
          return !isGuruKelas && !isGuruMapel && !isKepsek;
        });
      } else {
    ...
    if (isDinas && filterKondisi) {
      ...
      } else if (filterKondisi === "KepsekLama") {
        result = result.filter(item => {
          ...
          if (isKepsek && item.TMT_Kepsek_Formatted) {
            try {
              ...
              const years = new Date().getFullYear() - tmtK.getFullYear();
              return years >= 8;
            } catch (e) {}
          }
          return false;
    ...
    result.sort((a, b) => {
      ...
      // 2. Sekolah (Jenjang SD -> SMP -> TK, then school number numerically)
      const sekA = a.Sekolah || "";
      const sekB = b.Sekolah || "";
      const compSek = compareSchoolsByJenjangAndName(sekA, sekB);
      if (compSek !== 0) return compSek;
  ...
  const handleApplyDashboardFilters = (filters: {
    ...
    if (filters.filterKecamatan !== undefined) setFilterKecamatan(filters.filterKecamatan);
    if (filters.filterKondisi !== undefined) setFilterKondisi(filters.filterKondisi);
    if (filters.filterStatus !== undefined) setFilterStatus(filters.filterStatus);
    if (filters.filterBeban !== undefined) setFilterBeban(filters.filterBeban);
    if (filters.filterSertifikasi !== undefined) setFilterSertifikasi(filters.filterSertifikasi);

    // Navigate to Daftar PTK
    setActiveTab("data");
    setCurrentPage(1);
  };

  // Determine cascading schools based on selected Kecamatan and Jenjang
  const cascadingSchools = useMemo(() => {
    let filtered = schools;

    // Filter by Kecamatan first
    if (filterKecamatan) {
      filtered = filtered.filter(s => (s.kec || "").toUpperCase().trim() === filterKecamatan.toUpperCase().trim());
    }

    // Filter by Jenjang
    if (filterJenjang) {
      filtered = filtered.filter(s => {
        const sName = (s.nama || "").toUpperCase();
        if (filterJenjang === "TK") {
          return /\b(TK|TKN|TKS|PAUD|KB|BA|RA)\b/i.test(sName) || sName.startsWith("TK") || sName.startsWith("TKN") || sName.startsWith("TKS") || sName.startsWith("PAUD");
        }
        if (filterJenjang === "SD") {
    ...
    return filtered;
  }, [filterKecamatan, filterJenjang, schools]);
  ...
  // Excel triggered routine for Dinas
  const handleCetakLaporanSubmit = (config: any) => {
    if (session) {
      exportExcelDinas(filteredAndSortedGtkList, config);
    }
  };
  ...
  const handleExportExcelAction = () => {
    ...
    if (session?.role === "Admin Dinas") {
      ...
      const schoolData = schools.find(s => s.nama.toUpperCase().trim() === namaSekolah.toUpperCase().trim());
      exportExcelSekolah(
        filteredAndSortedGtkList, 
        namaSekolah, 
        schoolData?.alamat_sekolah, 
        schoolData?.jumlah_rombel
      );
    }
  };
            ...
            <main className="flex-grow p-4 sm:p-6 lg:p-8 w-full max-w-7xl mx-auto space-y-6">
              ...
              {activeTab === "dashboard" ? (
                ...
                <DashboardView 
                  ...
                  onApplyFiltersAndNavigate={handleApplyDashboardFilters}
                />
              ) : activeTab === "informasi" ? (
                // C. THE PAPAN INFORMASI & EDARAN PANEL
                <PapanInformasiView session={session} />
              ) : activeTab === "identitas" && session?.role === "Sekolah" ? (
                <SchoolIdentityView 
                  session={session} 
                  schools={schools} 
                  onRefreshSchools={fetchMetadata} 
                />
              ) : activeTab === "sekolah" && session?.role === "Admin Dinas" ? (
                <SchoolManagerView 
                  schools={schools} 
                  kecamatans={kecamatans} 
                  gtkList={gtkList}
                  ...
                  <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 border-b border-[#2d2420]/30 pb-4">
                    ...
                    <div className="flex flex-wrap items-center gap-1.5 w-full md:w-auto">
                      ...
                      <button
                        ...
                        title="Unduh format spreadsheet DUK Excel"
                      >
                        <FileSpreadsheet className="h-3.5 w-3.5" />
                        <span>{session.role === "Admin Dinas" ? "Unduh Laporan" : "Unduh Excel DUK"}</span>
                      </button>

                      {/* Unduh DUK Sekolah (Admin Dinas when filterSekolah is active) */}
                      {session?.role === "Admin Dinas" && filterSekolah && (
                        <button
                          onClick={() => {
                            if (filteredAndSortedGtkList.length === 0) {
                              return alert("Daftar unduhan kosong!");
                            }
                            const schoolData = schools.find(s => s.nama.toUpperCase().trim() === filterSekolah.toUpperCase().trim());
                            exportExcelSekolah(
                              filteredAndSortedGtkList, 
                              filterSekolah, 
                              schoolData?.alamat_sekolah, 
                          ...
                          className="inline-flex items-center gap-1.5 px-3.5 py-2 bg-amber-600 border border-amber-500 text-white hover:bg-amber-700 hover:border-amber-600 rounded-lg text-xs font-bold transition cursor-pointer animate-fade-in"
                          title="Unduh format DUK Excel Sekolah"
                        >
                          <FileSpreadsheet className="h-3.5 w-3.5" />
                          <span>Unduh DUK Sekolah</span>
                        </button>
                      )}
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-3.5">
                        ...
                        <div>
                          ...
                          <select
                            ...
                            className="w-full bg-[#13100e] border border-[#2d2420] rounded-lg px-2.5 py-1.5 text-xs text-stone-300 focus:outline-none focus:border-[#f25c05]/50 font-black"
                          >
                            <option value="">Semua Beban Tugas</option>
                            <option value="Kepsek_ALL">Kepala Sekolah + PLT. Kepala Sekolah</option>
                            ...
                            <option value="Lainnya">Lainnya / Staf</option>
                          </select>
                        </div>
                        ...
                        <div>
                          ...
                          >
                            ...
                            <option value="KepsekLama">Kepsek &gt; 8 Tahun</option>
                          </select>
                        </div>
                  ...
                  <div className="bg-[#1c1715] rounded-2xl shadow-xl border border-[#2d2420] overflow-hidden">
                    ...
                    {loading ? (
                      <div className="py-24 text-center text-stone-550 font-medium space-y-3">
                        ...
                        <p className="text-xs font-black uppercase tracking-widest text-[#f25c05]">Sinkronisasi Database kepegawaian...</p>
                      </div>
        ...
        <div className="fixed inset-0 z-[110] flex items-center justify-center bg-black/80 backdrop-blur-sm p-4 animate-fade-in">
          <div className="w-full max-w-sm rounded-xl overflow-hidden bg-[#181412] border border-[#2d2420] text-stone-100 shadow-2xl p-6 relative">
            ...
            <p className="text-[11px] text-rose-400 font-bold leading-relaxed mb-5">
              ⚠️ Tindakan ini tidak dapat dibatalkan, data akan langsung dihapus dari Database.
            </p>
