...
interface DashboardViewProps {
  ...
  onApplyFiltersAndNavigate?: (filters: {
...
export const DashboardView: React.FC<DashboardViewProps> = ({
  ...
  const bebanStats = useMemo(() => {
    ...
    gtkList.forEach(item => {
      ...
      if (t === "Guru Kelas") {
        stats["Guru Kelas"]++;
      } else if (t === "Guru Kelas TK") {
        stats["Guru Kelas TK"]++;
      } else if (t.startsWith("Guru Mapel") || t === "Guru PAI" || t === "Guru PJOK" || t === "Guru Bahasa Inggris") {
        stats["Guru Mapel"]++;
      } else if (t === "Kepala Sekolah") {
        stats["Kepala Sekolah"]++;
      } else if (t === "PLT. Kepala Sekolah") {
        stats["PLT. Kepala Sekolah"]++;
      } else {
        stats["Lainnya / Staf"]++;
      }
    });
      ...
      <div className={`grid grid-cols-1 ${isDinas ? "lg:grid-cols-3" : "md:grid-cols-2"} gap-6`}>
        ...
        {isDinas && (
          <div className="bg-[#1c1715] rounded-2xl border border-[#2d2420] p-6 shadow-xl flex flex-col justify-between min-h-[250px]">
            ...
            <div className="space-y-3.5 flex-grow h-[195px] max-h-[195px] overflow-y-auto pr-1 scrollbar-thin scrollbar-thumb-stone-800 scrollbar-track-transparent">
              {topKecDistribution.map((kec, index) => {
                ...
                return (
                  <div 
                    key={kec.name} 
                    onClick={() => onApplyFiltersAndNavigate?.({ filterKecamatan: kec.name })}
                    className="flex items-center justify-between p-2.5 rounded-xl bg-[#231a17]/50 border border-[#2d2420]/50 hover:bg-[#322521] hover:border-[#f25c05]/30 hover:scale-[1.02] transition cursor-pointer"
                  >
          ...
          <div className="grid grid-cols-4 gap-4 py-2 flex-grow items-end min-h-[140px] px-2">
            {[
              ...
              return (
                <div 
                  key={col.label} 
                  onClick={() => onApplyFiltersAndNavigate?.({ filterStatus: col.label })}
                  className="flex flex-col items-center group relative h-full justify-end cursor-pointer hover:scale-105 transition-transform"
                >
                  ...
                  <div className="w-5 sm:w-6 h-36 bg-[#251e1b] rounded-full overflow-hidden flex flex-col justify-end border border-[#322924] shadow-inner relative group-hover:border-[#f25c05]/30">
                    <div 
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="bg-[#1c1715] rounded-2xl border border-[#2d2420] p-6 shadow-xl flex flex-col justify-between min-h-[300px]">
          ...
          <div className="space-y-2 flex-grow">
            {[
              { name: "Guru Kelas SD", count: bebanStats["Guru Kelas"], desc: "Guru Kelas Sekolah Dasar" },
              { name: "Guru Kelas TK", count: bebanStats["Guru Kelas TK"] || 0, desc: "Guru Kelas TK" },
              { name: "Guru Mapel", count: bebanStats["Guru Mapel"], desc: "PAI, PJOK, B. Inggris dsb" },
              { name: "Kepala Sekolah", count: bebanStats["Kepala Sekolah"], desc: "Kepala Sekolah Definitif" },
              { name: "PLT. Kepala Sekolah", count: bebanStats["PLT. Kepala Sekolah"] || 0, desc: "Pelaksana Tugas Kepala Sekolah" },
              { name: "Lainnya / Staf", count: bebanStats["Lainnya / Staf"], desc: "Operator, TU & Penjaga" }
            ].map((task) => {
              const isSelected = task.count > 0;
              const filterVal = task.name === "Guru Kelas SD"
                ? "Guru Kelas"
                : task.name === "Kepala Sekolah" 
                  ? "Kepala Sekolah"
                  : task.name === "PLT. Kepala Sekolah"
                    ? "PLT. Kepala Sekolah"
                    : task.name === "Lainnya / Staf" 
                      ? "Lainnya" 
                      : task.name;
              return (
                <div 
                  key={task.name} 
                  onClick={() => onApplyFiltersAndNavigate?.({ filterBeban: filterVal })}
                  className="flex items-center justify-between p-2.5 rounded-xl bg-[#231a17]/30 border border-[#2d2420]/40 transition hover:border-[#f25c05]/40 hover:bg-[#2c211e] hover:scale-[1.01] cursor-pointer"
                >
                  <div className="flex items-center gap-3">
          ...
          <div className="space-y-3 flex-grow">
            {[
              ...
              const conditionMapping: Record<string, string> = {
                "PNS Pensiun": "Pensiun",
                "Mendekati Pensiun": "Mendekati",
                "Telat Naik Pangkat": "TelatNaik",
                "Telat KGB": "TelatKGB",
                "Akan KGB (<= 3 Bln)": "AkanKGB",
                "Kepsek > 8 Tahun": "KepsekLama"
              };
              return (
                <div 
                  key={alertItem.name} 
                  onClick={() => {
                    const cond = conditionMapping[alertItem.name];
                    if (cond) onApplyFiltersAndNavigate?.({ filterKondisi: cond });
                  }}
                  className="flex items-center justify-between p-2.5 rounded-xl bg-[#231a17]/30 border border-[#2d2420]/40 transition hover:bg-[#2c211e] hover:border-[#f25c05]/30 hover:scale-[1.01] cursor-pointer"
                >
                  <div className="flex items-center gap-3">
        ...
        <div className="bg-[#1c1715] rounded-2xl border border-[#2d2420] p-6 shadow-xl flex flex-col justify-between min-h-[300px]">
          ...
          <div className="flex flex-col sm:flex-row items-center gap-5 py-4 flex-grow justify-center">
            ...
            <div 
              onClick={() => onApplyFiltersAndNavigate?.({ filterSertifikasi: "Ya" })}
              className="relative w-28 h-28 flex items-center justify-center cursor-pointer hover:scale-105 transition-transform"
              title="Filter Sudah Sertifikasi"
            >
            ...
            <div className="space-y-2 text-center sm:text-left">
              <div 
                onClick={() => onApplyFiltersAndNavigate?.({ filterSertifikasi: "Ya" })}
                className="cursor-pointer hover:bg-[#2c211e] p-1.5 rounded-lg transition border border-transparent hover:border-[#f25c05]/20"
                title="Filter Sudah Sertifikasi"
              >
              ...
              <div 
                onClick={() => onApplyFiltersAndNavigate?.({ filterSertifikasi: "Belum" })}
                className="border-t border-[#2d2420] pt-1.5 cursor-pointer hover:bg-[#2c211e] p-1.5 rounded-lg transition border hover:border-[#f25c05]/20 mt-1"
                title="Filter Belum Sertifikasi"
              >
